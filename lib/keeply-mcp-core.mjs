...
export async function route(method, params = {}) {
...
export async function handleJsonRpc(message) {
  ...
  try {
    return {
      jsonrpc: "2.0",
      id: message.id,
      result: await route(message.method, message.params || {})
    };
  } catch (error) {
    return jsonRpcError(message.id, error.code || -32000, error.message || "Server error", error.details);
  }
}
