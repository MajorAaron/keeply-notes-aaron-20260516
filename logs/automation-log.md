...
- Notes and tasks now show compact `Urgent`, `ASAP`, or `Critical` pills when text includes urgency cues such as urgent, ASAP, time-sensitive, critical, P0, or high priority; negated phrases such as `not urgent` and `non-urgent` stay quiet.
- Updated What's New metadata with latest id `2026-05-22-urgency-badges`, title `Urgency Badges`, and 3 user-facing bullets.
- UI/code areas touched: shared deterministic urgency detector in `item-urgency-meta.mjs`, note/task metadata rendering in `app.js`, note/task templates in `index.html`, badge styling in `styles.css`, release metadata, package test wiring, and `test/item-urgency-meta.test.mjs`. No new dependencies or Netlify functions were added.
- AI/API behavior: no new AI endpoint or API key usage was added; the improvement is deterministic local urgency-cue detection only.
- Verification: focused `node --check item-urgency-meta.mjs && node --test test/item-urgency-meta.test.mjs` passed with 5 tests; full `npm test` passed with 248 passing tests; `git diff --check` passed; `node scripts/check-release-updates.mjs` ran successfully and as part of `npm test`.
- Local smoke verification: `HOST=127.0.0.1 PORT=4216 npm start` served `http://127.0.0.1:4216/` with HTTP 200. Browser verification confirmed the `Urgency Badges` What's New popup/dismissal persistence, dynamic-imported local urgency metadata returning `Critical` for a P0 critical ASAP cue and hidden metadata for `not urgent`, no document-level horizontal overflow, and no browser console errors. The local server was stopped after verification.
- Production verification: deploy permalink loaded, exposed latest update id `2026-05-22-urgency-badges` in the What's New popup, dismissed to `keeply-last-seen-update`, dynamically imported `item-urgency-meta.mjs` and returned `Critical` for a P0 critical ASAP cue while keeping `not urgent` hidden, reported no document horizontal overflow, and had no browser console errors. Production seed data did not include visible urgency cards, so the deployed helper/import path was smoke-tested directly without mutating remote data.
...
- Git status: feature commit `ed93115` (`Add urgency context badges`) was pushed to `origin main`; unrelated untracked `backups/` was left untouched.
- Netlify production deploy succeeded with `npx netlify deploy --prod --dir . --no-build --json`. Feature deploy `6a108e21bb4a0f387d3befa8` is live at `https://keeply-notes-aaron-20260516.netlify.app`; deploy permalink: `https://6a108e21bb4a0f387d3befa8--keeply-notes-aaron-20260516.netlify.app`

## 2026-05-22 10:05:48 MDT

- Added mobile-first Quote Badges for faster scanning of quoted feedback, excerpts, and pasted references across notes and tasks.
- Notes and tasks now show compact `Quote` / `N quotes` pills when text includes markdown blockquotes or smart/straight quoted excerpts; apostrophes inside words stay quiet to avoid noisy badges.
- Updated What's New metadata with latest id `2026-05-22-quote-badges`, title `Quote Badges`, and 3 user-facing bullets.
- UI/code areas touched: shared deterministic quote detector in `item-quote-meta.mjs`, note/task metadata rendering in `app.js`, note/task templates in `index.html`, badge styling in `styles.css`, release metadata, package test wiring, and `test/item-quote-meta.test.mjs`. No new dependencies or Netlify functions were added.
- AI/API behavior: no new AI endpoint or API key usage was added; the improvement is deterministic local quote/excerpt detection only.
- Verification: focused `node --check item-quote-meta.mjs && node --test test/item-quote-meta.test.mjs` passed with 5 tests; full `npm test` passed with 248 passing tests; `git diff --check` passed; `node scripts/check-release-updates.mjs` ran as part of `npm test`.
- Local smoke verification: `HOST=127.0.0.1 PORT=4215 npm start` served `http://127.0.0.1:4215/` with HTTP 200. Browser verification confirmed the `Quote Badges` What's New popup/dismissal persistence, dynamic-imported local quote metadata returning `2 quotes`, no document-level horizontal overflow, and no browser console errors. The local server was stopped after verification.
- Production verification: deploy permalink loaded, exposed latest update id `2026-05-22-quote-badges` in the What's New popup, dismissed to `keeply-last-seen-update`, dynamically imported `item-quote-meta.mjs` and returned `2 quotes`, reported no document horizontal overflow, and had no browser console errors. Production seed data did not include visible quoted cards, so the deployed helper/import path was smoke-tested directly without mutating remote data.
- Secret scan of changed app/index/style/metadata/helper/test/package files found no committed `ANTHROPIC_API_KEY`, long `sk-` key, secret assignment, token assignment, or API key assignment; existing broad-regex matches for `task-composer-priority-preset` and `task-scheduled-time-badges` were reviewed as false positives.
- Git status: feature commit `ace5ad5` (`Add quote context badges`) was pushed to `origin main`; unrelated untracked `backups/` was left untouched.
- Netlify production deploy succeeded with `npx netlify deploy --prod --dir . --no-build --json`. Feature deploy `6a107ec1eaae510c172113a7` is live at `https://keeply-notes-aaron-20260516.netlify.app`; deploy permalink: `https://6a107ec1eaae510c172113a7--keeply-notes-aaron-20260516.netlify.app`
...
- Task capture now understands `next Friday`, `next Monday`, and other `next <weekday>` phrases in both single-task titles and pasted task lists, sets the matching next-week due date, and strips the hint text from saved titles.
- Updated What's New metadata with latest id `2026-05-21-next-weekday-task-hints`, title `Next Weekday Task Hints`, and 3 user-facing bullets.
- UI/code areas touched: deterministic task hint parsing in `task-bulk-entry.mjs`, single-task capture coverage through `task-capture-hints.mjs`, release metadata, and parser/capture tests in `test/task-bulk-entry.test.mjs` plus `test/task-capture-hints.test.mjs`. No new dependencies or Netlify functions were added.
- AI/API behavior: no new AI endpoint or API key usage was added; the improvement is deterministic local task date parsing only.
- Verification: full `npm test` passed with 235 passing tests; `git diff --check` passed; `node scripts/check-release-updates.mjs` ran as part of `npm test`.
- Local smoke verification: `HOST=127.0.0.1 PORT=4202 npm start` served `http://127.0.0.1:4202/` with HTTP 200. Browser verification confirmed the `Next Weekday Task Hints` What's New popup/dismissal persistence, dynamic-imported local parsing for `Send budget next Friday @work` to title `Send budget`, Work label, and due date `2026-05-29`, no document-level horizontal overflow, and no browser console errors. The local server was stopped after verification.
- Production verification: deploy permalink loaded, exposed latest update id `2026-05-21-next-weekday-task-hints` in the What's New popup, dismissed to `keeply-last-seen-update`, dynamically imported `task-bulk-entry.mjs` and parsed `Draft launch notes next Friday @work` to title `Draft launch notes`, Work label, and due date `2026-05-29`, reported no document horizontal overflow, and had no browser console errors.
...
- Git status: feature commit `2e0e758` (`Add next weekday task hints`) was pushed to `origin main`; unrelated untracked `backups/` was left untouched.
- Netlify production deploy succeeded with `npx netlify deploy --prod --dir . --no-build --json`. Feature deploy `6a0fabaf643688c1d1164c5e` is live at `https://keeply-notes-aaron-20260516.netlify.app`; deploy permalink: `https://6a0fabaf643688c1d1164c5e--keeply-notes-aaron-20260516.netlify.app`
