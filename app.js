...
import { getTaskRepeatMeta } from "./task-repeat-meta.mjs";
...
const state = {
  ...
  notePin: savedPreferences.notePin,
  query: "",
  color: "sun",
...
const els = {
  ...
  notePinFilters: document.querySelector("#notePinFilters"),
...
function getVisibleNotes() {
  ...
  return state.notes
    ...
    .filter((note) => state.view !== "active" || matchesNotePinFilter(note, state.notePin))
...
function render() {
  ...
  els.othersHeading.hidden = showTasks ? tasks.length === 0 : others.length === 0;
  els.pinnedNotes.hidden = showTasks;
  els.notesGrid.hidden = showTasks;
  els.taskList.hidden = !showTasks && !showMixedArchive;
  els.notePinFilters.hidden = state.view !== "active";
...
function getLabelCountItems() {
  ...
  return state.notes.filter((note) => note.status === "active" && matchesNoteColorFilter(note, state.noteColor) && matchesNotePinFilter(note, state.notePin));
}
...
function renderNoteColorFilters() {
  const countableNotes = state.notes.filter((note) => note.status === "active" && matchesNotePinFilter(note, state.notePin) && (state.label === "all" || note.label === state.label));
...
function renderNotePinFilters() {
  const countableNotes = state.notes.filter(
    (note) => note.status === "active" && matchesNoteColorFilter(note, state.noteColor) && (state.label === "all" || note.label === state.label)
  );
  const counts = getNotePinFilterCounts(countableNotes);

  els.notePinFilters.querySelectorAll(".note-pin-filter").forEach((button) => {
    const filter = normalizeNotePinFilter(button.dataset.pinFilter);
    const active = filter === state.notePin;
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", String(active));
    button.querySelector(".task-filter-count").textContent = counts[filter] ?? 0;
  });
...
function renderNote(note) {
  ...
  const mentionMeta = getItemMentionMeta(noteSearchText);
  const mentionMetaElement = node.querySelector(".note-mention-meta");
  mentionMetaElement.hidden = !mentionMeta.available;
  mentionMetaElement.textContent = mentionMeta.label;
  mentionMetaElement.setAttribute("aria-label", mentionMeta.ariaLabel);
  mentionMetaElement.dataset.tone = mentionMeta.tone || "";
  ...
  const travelMeta = getItemTravelMeta(noteSearchText);
  const travelMetaElement = node.querySelector(".note-travel-meta");
  travelMetaElement.hidden = !travelMeta.available;
...
function renderTask(task) {
  ...
  tagMetaElement.setAttribute("aria-label", tagMeta.ariaLabel);
  tagMetaElement.dataset.tone = tagMeta.tone || "";
  const mentionMeta = getItemMentionMeta(taskSearchText);
  const mentionMetaElement = node.querySelector(".task-mention-meta");
  mentionMetaElement.hidden = !mentionMeta.available;
  mentionMetaElement.textContent = mentionMeta.label;
  mentionMetaElement.setAttribute("aria-label", mentionMeta.ariaLabel);
  mentionMetaElement.dataset.tone = mentionMeta.tone || "";
  ...
  urgencyMetaElement.setAttribute("aria-label", urgencyMeta.ariaLabel);
  urgencyMetaElement.dataset.tone = urgencyMeta.tone || "";
  const travelMeta = getItemTravelMeta(taskSearchText);
  const travelMetaElement = node.querySelector(".task-travel-meta");
  travelMetaElement.hidden = !travelMeta.available;
  travelMetaElement.textContent = travelMeta.label;
  travelMetaElement.setAttribute("aria-label", travelMeta.ariaLabel);
  travelMetaElement.dataset.tone = travelMeta.tone || "";
  ...
  const timeEstimateMeta = getTaskTimeEstimateMeta(task);
  const timeEstimateMetaElement = node.querySelector(".task-time-estimate-meta");
  ...
  const repeatMeta = getTaskRepeatMeta(task);
  const repeatMetaElement = node.querySelector(".task-repeat-meta");
  repeatMetaElement.hidden = !repeatMeta.available;
  repeatMetaElement.textContent = repeatMeta.label;
  repeatMetaElement.setAttribute("aria-label", repeatMeta.ariaLabel);
  repeatMetaElement.dataset.tone = repeatMeta.tone;
...
document.querySelectorAll(".note-pin-filter").forEach((button) => {
  button.addEventListener("click", () => {
    state.notePin = normalizeNotePinFilter(button.dataset.pinFilter);
    saveViewPreferences();
    render();
  });
});
